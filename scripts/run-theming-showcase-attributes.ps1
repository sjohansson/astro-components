#!/usr/bin/env pwsh
# Run the data-attribute theming-showcase example locally.
# Performs a full repo check (build + lint + typecheck + tests) at the repo root,
# then starts the Astro dev server in examples/theming-showcase-attributes.
# Usage: .\scripts\run-theming-showcase-attributes.ps1 [-SkipCheck]
...
$showcaseDir = Join-Path $repoRoot "examples/theming-showcase-attributes"
